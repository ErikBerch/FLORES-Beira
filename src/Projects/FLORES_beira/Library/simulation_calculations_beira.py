...
def run_hydraulic_calculation(region_layout, hydraulic, strategy):
    ...
    time = np.arange(0, max_t+0.1, 0.1) * 3600                # timestep in secondes*10 is shown after the multiplier (now at 360)
...
def tolist(input_string, splitby):
    """
    ...
    if type(input_string) == str:
        return list(map(int, input_string.split(splitby)))
    else:
        str_variable = str(int(input_string))
        return list(map(int, str_variable.split(splitby)))
