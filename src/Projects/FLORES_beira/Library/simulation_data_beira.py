    ...
    for row in range(0, len(measures_source)):
        ...
        if tmp_measure['Type'] == 'Structural':
            if tmp_measure['STR_Land/Water'] == 'Land':
                measures_master[tmp_measure['Name']] = StructuralMeasureLand(tmp_measure['Name'], tmp_measure['Code'], tmp_measure['Layer'],
                                                                             tmp_measure['Location'], float(tmp_measure['Constant costs']), float(tmp_measure['Variable costs']), irribaren=float(tmp_measure['STR_irribaren']))
            elif tmp_measure['STR_Land/Water'] == 'Water':
                measures_master[tmp_measure['Name']] = StructuralMeasureWater(tmp_measure['Name'], tmp_measure['Code'], tmp_measure['Layer'],
                                                                              tmp_measure['Location'], float(tmp_measure['Constant costs']), float(tmp_measure['Variable costs']), irribaren=float(tmp_measure['STR_irribaren']))
            else:
                print("wrong structural location chosen: " + str(tmp_measure['Measure']))
        elif tmp_measure['Type'] == 'Nature-based solution':
            measures_master[tmp_measure['Name']] = NatureBased(tmp_measure['Name'], tmp_measure['Code'], tmp_measure['Location'],
            ...
            measures_master[tmp_measure['Name']] = EmergencyMeasure(tmp_measure['Name'], tmp_measure['Code'], change_emergency_dict,
            ...
            measures_master[tmp_measure['Name']] = DrainageMeasure(tmp_measure['Name'], tmp_measure['Code'], change_drainage_dict,
                                                                   tmp_measure['Constant costs'])
            ...
            measures_master[tmp_measure['Name']] = RetentionMeasure(tmp_measure['Name'], tmp_measure['Code'], change_retention_dict,
                                                                    tmp_measure['Constant costs'])
...
def load_damage_curves(file, continent, country, modeltype, exchange):
    ...
    df_damage_function = pd.read_excel(xls, 'Damage functions', header=2, usecols=8)
    factors = {}
    maxvalues = {}
    ...
    maxvalues['Residential'] = pd.read_excel(xls, 'MaxDamage-Residential', header=0)[modeltype][country]
    maxvalues['Commercial'] = pd.read_excel(xls, 'MaxDamage-Commercial', header=0)[modeltype][country]
    maxvalues['Industrial'] = pd.read_excel(xls, 'MaxDamage-Industrial', header=0)[modeltype][country]
    maxvalues['Agriculture'] = pd.read_excel(xls, 'MaxDamage-Agriculture', header=0)['Value Added/Hectare\n(avg 2008-2012)'][country]
    maxvalues['Infrastructure'] = pd.read_excel(xls, 'MaxDamage-Infrastructure', header=0,usecols=2,index_col=0)['GDP per capita (2010 US$)'][country]
